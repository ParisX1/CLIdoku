...
def play(board):
    ...
    while True:
        ...
        # Infer Solution
        elif (userInput[0] == 'i' or userInput[0] == 'infer'):
            printBoard(inferred(board))
        ...
        # Generate New Board
        elif (userInput[0] == 'g' or userInput[0] == 'generate'):
            getSize = userInput[1:]
            sizeInt = int(getSize[0])
            board = generate(sizeInt)
            printBoard(board)
...
def inferred(board):
    ...
    Returns:
        int array: new Soduko board with all values field from input board plus all values that can be
...
- Put in comments about what forward and backward single rules are !
- Mention that indexing is zero based
- Finish 'help' function
- Generate vs new board in inputs ?
- Put the functions in some kind of proper order

- Check id any unused variables to remove
- Check the big boards are working
- Some kind of warning when making big boards that it can take some time
- Check all the functionality - hint, undo, solve, infer, new, etc

- Load to GitHub and create a proper ReadMe

---

- (done) Remove "sudokus = [[]]"
- (done) Remove all the "import ..." and have once at the top
- (done) Remove all the useless comments
- (done) Check variables_that_have_underscores!
- (done) Put proper function docstrings
